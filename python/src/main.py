...
class Timings:
    def __init__(self):
        ...
        self.interp_times: list[float] = []
        self.draw_times: list[float] = []
        self.errors = 0

    def __str__(self):
        return (
                f"Request times, average: {self.mean_request_time_ms:.2f}ms, samples: : {len(self.request_times)}, "
                f"Parse times, average:   {self.mean_parse_time_ms:.2f}ms, samples: : {len(self.parse_times)}, "
                f"Read times, average:    {self.mean_read_time_ms:.2f}ms, samples: : {len(self.read_times)}, "
                f"Interp times, average:  {self.mean_interp_time_ms:.2f}ms, samples: : {len(self.interp_times)}, "
                f"Draw times, average:    {self.mean_draw_time_ms:.2f}ms, samples: : {len(self.draw_times)}, "
                f"Errors: {self.errors}"
        )

    @property
    def mean_request_time_ms(self) -> float:
        return statistics.mean(self.request_times) * 1000.0

    @property
    def mean_parse_time_ms(self) -> float:
        return statistics.mean(self.parse_times) * 1000.0

    @property
    def mean_read_time_ms(self) -> float:
        return statistics.mean(self.read_times) * 1000.0

    @property
    def mean_interp_time_ms(self) -> float:
        return statistics.mean(self.interp_times) * 1000.0

    @property
    def mean_draw_time_ms(self) -> float:
        return statistics.mean(self.draw_times) * 1000.0
...
def run_interpolation_process(sensor_url:str, frames:int, enable_agc:bool=False)->Timings:
    #####################################
    ...
    if True:
        ...
        for frame_count in range(0, frames or 1000000000):

            read_start_time = time.perf_counter()

            # Hard-coded URL depends on server - ideally change this to dynamic
            request = requests.get(sensor_url, timeout=5)

            request_end_time = time.perf_counter()
            timings.request_times.append(request_end_time - read_start_time)

            data = request.json()

            timings.parse_times.append(time.perf_counter() - request_end_time)
            ...
            timings.read_times.append(read_end_time - read_start_time)
            timings.interp_times.append(interp_end_time - read_end_time)
            timings.draw_times.append(draw_end_time - interp_end_time)
            ...
            if enable_agc and not time.perf_counter() - last_agc_adjustment >= AGC_TIME:
...
def main(args: argparse.Namespace):
    timings: Timings = run_interpolation_process(sensor_url=sensor_api_url, frames=args.frames)

    if args.timing:
        print(f"Request: {timings.mean_request_time_ms:.2f}ms")
        print(f"Parse:   {timings.mean_parse_time_ms:.2f}ms")
        print(f"Read:    {timings.mean_read_time_ms:.2f}ms")
        print(f"Interp:  {timings.mean_interp_time_ms:.2f}ms")
        print(f"Draw:    {timings.mean_draw_time_ms:.2f}ms")
        print(f"Errors:  {timings.errors}")
...
if __name__ == "__main__":
    ...
    sensor_api_url:str = f"http://{args.server}:8000/sensor/0/data"
